...
export type SavedForecast={id?:string;game:Game;target_round:number;target_date:string|null;model:ModelMode;picks:string[];scores:number[];purchase_type?:'straight';stake_yen?:number;return_yen?:number|null;roi_percent?:number|null;created_at?:string;settled_at?:string|null;winning_number?:string|null;straight_hit?:boolean|null;box_hit?:boolean|null;best_digit_match?:number|null;status?:'pending'|'settled'};
...
export async function saveForecast(game:Game,targetRound:number,targetDate:string|null,model:ModelMode,predictions:Prediction[]){
  if(!persistenceEnabled())return null;
  const payload={game,target_round:targetRound,target_date:targetDate,model,picks:predictions.map(p=>p.number),scores:predictions.map(p=>Number.isFinite(p.score)?p.score:0),purchase_type:'straight',stake_yen:predictions.length*200,status:'pending'};
  const rows=await request<SavedForecast[]>('forecasts?on_conflict=game,target_round,model',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify(payload)});return rows?.[0]??null
}
